...
const ProductList = ({ selectedCategory, filters }) => {
  ...
  const applyFilters = (product) => {
    ...
    if (filters.price) {
      switch (filters.price) {
        ...
        case "2-4":
          passFilters =
            passFilters &&
            product.newPrice >= 2000000 &&
            product.newPrice <= 4000000;
          break;
        case "4-7":
          passFilters =
            passFilters &&
            product.newPrice >= 4000000 &&
            product.newPrice <= 7000000;
          break;
        case "7-13":
          passFilters =
            passFilters &&
            product.newPrice >= 7000000 &&
            product.newPrice <= 13000000;
          break;
        case "13-20":
          passFilters =
            passFilters &&
            product.newPrice >= 13000000 &&
            product.newPrice <= 20000000;
          break;
        case ">20":
    ...
    if (filters.battery) {
      switch (filters.battery) {
        ...
        case "4000-5000":
          passFilters =
            passFilters && product.pin >= 4000 && product.pin <= 5000;
          break;
        case ">5000":
  ...
  const products = selectedCategory
    ? allproducts.filter(
        (product) =>
          product.category === selectedCategory && applyFilters(product)
      )
